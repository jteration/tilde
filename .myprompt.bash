set_prompt() {
	...
	# Colours
	local red green yellow blue magenta cyan orange grey violet lgrey no_colour white
	red=$(tput setaf 1)
	green=$(tput setaf 2)
	yellow=$(tput setaf 3)
	blue=$(tput setaf 4)
	...
	if ((${#cwd_arr[@]} > 1)); then
		printf -v __cwd '%.5s/' "${cwd_arr[@]:0:$((${#cwd_arr[@]} - 1))}"
		__cwd+=${cwd_arr[-1]%/}
	fi
	...
	# Set rest of prompt
	# User and hostname
	PS1+="\\[$white\\]\$__un\\[$orange\\]@\\[$yellow\\]\$__hn\\[$orange\\] "

	# Working directory and coloured prompt
	PS1+="\\[$violet\\]\$__cwd\\[$sign_col\\]\n\\$\\[$no_colour\\] "
